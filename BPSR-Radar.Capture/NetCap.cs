...
public enum ObservedPacketFlow : byte
{
    ClientToServerCall,
    ClientToServerFrameUp,
    ServerToClientNotify,
    ServerToClientReturn,
}

public delegate void RawPacketObserverDelegate(ObservedPacketFlow flow, ulong serviceId, uint methodId, ReadOnlySpan<byte> payload, DateTime packetTimeUtc);

public class NetCap
{
    private const uint MaxGameMessageSize = 64u * 1024u * 1024u;
    ...
    public RawPacketObserverDelegate? RawPacketObserver = null;
    ...
    private bool IsReplayMode => !string.IsNullOrEmpty(Config.ReplayPcapPath);
    ...
    public void Start()
    {
        if (IsReplayMode)
        {
            CaptureDevice = new CaptureFileReaderDevice(Config.ReplayPcapPath!);
            CaptureDevice.Open();
        }
        else
        {
        ...
        if (!IsReplayMode && !string.IsNullOrEmpty(Config.RecordPcapPath))
        {
            Directory.CreateDirectory(Path.GetDirectoryName(Config.RecordPcapPath!)!);
            PcapWriter = new CaptureFileWriterDevice(Config.RecordPcapPath!);
            PcapWriter.Open(CaptureDevice);
            Log.Information("Recording packets to {Path}", Config.RecordPcapPath);
        }
    ...
    private void DeviceOnOnPacketArrival(object sender, PacketCapture e)
    {
        ...
        if (PcapWriter != null)
        {
            try { PcapWriter.Write(rawPacket); } catch { }
        }

        if (IsReplayMode && Config.ReplayRealtime)
        {
    ...
    private void OnNewConnection(TcpReassembler.TcpConnection conn)
    {
        var task = Task.Factory.StartNew(async () =>
        {
            ...
            while (conn.IsAlive && !CancelTokenSrc.IsCancellationRequested && !conn.CancelTokenSrc.IsCancellationRequested) {
                ...
                if (len < 6 || len > MaxGameMessageSize)
                {
                    Log.Logger.Warning(
                        "Invalid game message length from {EndPoint} to {DestEndPoint}: {Length}; maximum is {MaxGameMessageSize}",
                        conn.EndPoint, conn.DestEndPoint, len, MaxGameMessageSize);
    ...
    private void ParseNotify(ReadOnlySpan<byte> data, bool isCompressed, DateTime lastPacketTime)
    {
        ...
        RawPacketObserver?.Invoke(ObservedPacketFlow.ServerToClientNotify, serviceUuid, methodId, msgData, lastPacketTime);
    ...
    private void ParseCall(ReadOnlySpan<byte> data, bool isCompressed, DateTime lastPacketTime)
    {
        ...
        RawPacketObserver?.Invoke(ObservedPacketFlow.ClientToServerCall, proxyServiceId, proxyMethodId, msgData, lastPacketTime);
    ...
    private void ParseFrameUp(ReadOnlySpan<byte> data, bool isCompressed, DateTime lastPacketTime)
    {
        ...
        if (data.Length < 26)
        {
            ...
            Log.Logger.Warning("ParseFrameUp is too small: length {Length}", data.Length);
            return;
        }
        ...
        while (offset < data.Length)
        {
            ...
            var remaining = data.Length - frameStart;
            if (remaining < 4)
            {
                Log.Logger.Warning(
                    "ParseFrameUp nested frame header is truncated at offset {Offset}: remaining length {RemainingLength}",
                    frameStart, remaining);
                return;
            }

            var length = BinaryPrimitives.ReadUInt32BigEndian(data[offset..]);
            if (length > int.MaxValue || length < 14 || length > (uint)remaining)
            {
                Log.Logger.Warning(
                    "ParseFrameUp nested frame has invalid length {Length} at offset {Offset}; remaining length {RemainingLength}",
                    length, frameStart, remaining);
                return;
            }

            var endPos = frameStart + (int)length;
            offset += 4;
            ...
            if (flags == 2)
            {
                ...
                {
                    Log.Logger.Warning(
                        "ParseFrameUp nested frame with flags {Flags} is too small: length {Length}, minimum {MinimumLength}",
                        flags, length, 22);
                    return;
                }
                ...
                RawPacketObserver?.Invoke(ObservedPacketFlow.ClientToServerFrameUp, proxyServiceId, proxyMethodId, msgData, lastPacketTime);
    ...
    private void ParseReturn(ReadOnlySpan<byte> data, bool isCompressed, DateTime lastPacketTime)
    {
        ...
        {
            RawPacketObserver?.Invoke(ObservedPacketFlow.ServerToClientReturn, frameUp.ServiceId, frameUp.MethodId, msgData[protoStart..], lastPacketTime);
